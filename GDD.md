...
- [x] React Native + Expo プロジェクトセットアップ
- [x] モンスターパラメータシステム（HP/ATK/DEF/規律）
- [x] TPシステム（3系統、トレーニングメニュー）
- [x] 自動バトルエンジン
- [x] WPエンカウントシステム
- [x] 進化システム（条件判定＋分岐）
- [x] 世代引き継ぎ（「記憶」装備生成）
...
- [x] Expo Router 画面遷移
- [x] LCD液晶風レトロUI (LcdFrame + MonsterSprite)
- [x] メイン画面・バトル・トレーニング・ワールド選択・図鑑・設定・ステータス
- [x] セーブスロット選択画面 (3スロット)
- [x] TP配分画面・世代履歴画面

### Phase 3: 運動データ連携 ✅ 完了

- [x] Health Connect統合（歩数・階段→WP）
- [x] RideMetrics API連携（ゾーン別擬似TSS→TP計算）
- [x] PMCボーナス計算ロジック
- [x] 食事質問＋規律変動システム
- [x] 通知システム
- [x] 手動SYNCボタン、歩数救済（最大3日分）

### Phase 4: データ拡張・バランス ✅ 完了

- [x] モンスター拡張（100→150体、進化ツリー214パス）
- [x] ワールド拡張（8→10ワールド）
- [x] 世代履歴UI
- [x] バランスリライト: 20%進化ブースト、段階別訓練係数、装備なし進化判定
- [x] Monte Carloバランスシミュレーション (339テスト通過)

### Phase 5: Android実機対応 🔧 進行中

- [x] EAS Build (preview profile → APK)
- [x] Health Connect権限修正（init/permission分離、クラッシュ修正）
- [x] フォントサイズスケーリング (~1.4x)
- [x] 全画面レイアウト (flex:1)
- [ ] 実機テスト最終確認
- [ ] 初期ボーナスWP削除
...
以下の数値は開発中のバランス調整で GDD 記載値から変更されている。実装値は `CLAUDE.md` および各ソースファイル内の定数が正:

| 項目 | GDD記載値 | 実装値 |
|------|----------|--------|
| エンカウントWP消費 | 3,000 WP | **2,000 WP** |
| エンカウント空振り率 | 20% | **0% (廃止)** |
| ワールド数 | 8 | **10** |
| モンスター総数 | 40〜50 | **150** |
| 進化条件 | GDD 6.2表 | **CLAUDE.md の進化条件テーブル参照** |
| PMC基礎TP | 8 | **35** |
| 寿命基礎値 | 7〜9日 | **5〜9日 (6段階バケット)** |
| 食事延長 | 20%確率で+0.5日 | **毎食確定 0.03〜0.25日** |
